...
export default function AdminEditActivityPage() {
  ...
  useEffect(() => {
    ...
    const loadActivity = async () => {
      if (!activityId || isLoadingAuth) return
      ...
      try {
        ...
        // 首先尝试获取单个活动（如果API支持）
        try {
          const singleResponse = await fetch(`/api/admin/activities/${activityId}`)
          if (singleResponse.ok) {
            // 如果API支持获取单个活动
            const result = await singleResponse.json()
            activity = result.activity || result.data || result
            if (activity && activity.id) {
              // 成功获取单个活动，跳过获取所有活动的步骤
              console.log('Successfully loaded single activity via API')
            } else {
              activity = null
            }
          } else if (singleResponse.status !== 404) {
            // 如果不是404错误，记录错误但继续尝试获取所有活动
            console.warn('Failed to load single activity, status:', singleResponse.status)
          }
        } catch (singleError) {
          // 如果获取单个活动失败，忽略错误，继续尝试获取所有活动
          console.log('Single activity API not available, falling back to list API')
        }

        // 如果获取单个活动失败，则获取所有活动并过滤
        if (!activity) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        setError(result.message || result.error || '更新活动失败')
        setIsSubmitting(false)
        return
      }
  ...
  // 如果正在加载认证信息，显示加载状态
  if (isLoadingAuth || isLoadingActivity) {
    return (
      <div style={{ 
        minHeight: '100vh', 
        ...
        <div style={{ maxWidth: '56rem', margin: '0 auto', padding: '1rem 1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              ...
              <div>
                <h1 style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'white', margin: 0 }}>编辑活动</h1>
                <p style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: '0.875rem', margin: 0 }}>修改活动详情</p>
              </div>
            </div>
          </div>
        </div>
