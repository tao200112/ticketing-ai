...
export default function AdminMerchantsPage() {
  ...
  const handleRegionUpdate = async (merchantId, nextRegionId) => {
    if (!merchantId || !nextRegionId) {
      return
    }
    ...
    try {
      const response = await fetch(`/api/admin/merchants/${merchantId}`, {
        ...
        body: JSON.stringify({ region_id: nextRegionId }),
      })
      ...
      setToast({ type: 'success', message: 'Merchant region updated' })
      await loadMerchants()
    } catch (err) {
      console.error('Failed to update merchant region', err)
                      ...
                      <td style={{ padding: '16px 20px' }}>
                        {regionsLoading ? (
                          ...
                          <select
                            ...
                            onChange={(e) => handleRegionUpdate(merchant.id, e.target.value)}
                            style={{
                              width: '100%',
                              padding: '8px 12px',
                              borderRadius: '8px',
