...
export async function getServerAuthIdentity() {
  try {
    ...
    if (!getServerUser || typeof getServerUser !== 'function') {
      console.error('[getServerAuthIdentity] getServerUser is not a function')
      return null
    }
    ...
    if (error.stack) {
      console.error('[getServerAuthIdentity] Error stack:', error.stack)
    }
    return null
  }
}
