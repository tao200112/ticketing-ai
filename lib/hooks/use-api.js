  ...
  const fetchEvents = useCallback(
    async (slugOverride) => {
      ...
      try {
        setLoading(true);
        setError(null);
        console.log('🔍 useEvents: 开始获取活动数据', { regionSlug: effectiveSlug || 'all' });

        const query = effectiveSlug
          ? `/api/events?region=${encodeURIComponent(effectiveSlug)}`
          : '/api/events';
  ...
  useEffect(() => {
    ...
    return () => {
      if (typeof window !== 'undefined' && window.refreshEvents) {
        window.refreshEvents = undefined;
      }
    };
  }, [regionSlug, fetchEvents]);

  return { data, loading, error, refresh: fetchEvents, refetch: fetchEvents };
}
