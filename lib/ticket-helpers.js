...
export function getComboTicketKinds(priceName, ticketKind = null) {
  ...
  if (ticketKind === 'combo' || ticketKind === 'ENTRY_COMBO' || ticketKind === 'DRINK_COMBO') {
    console.log(`[getComboTicketKinds] ticketKind is "${ticketKind}", returning default combo kinds`)
    return getComboTicketKindsDefault()
  }
  
  if (!isComboTicket(priceName, ticketKind)) {
    console.log(`[getComboTicketKinds] Not a combo ticket (priceName: "${priceName}", ticketKind: "${ticketKind}"), returning empty array`)
    return []
  }
  ...
  const name = priceName ? priceName.toLowerCase().trim() : ''
  const kinds = []
  ...
  const result = kinds.length > 0 ? kinds : getComboTicketKindsDefault()
  console.log(`[getComboTicketKinds] Returning ${result.length} ticket kinds:`, result)
  return result
}
